    ...
    <div>
      <svg viewBox={`0 0 ${size} ${size}`} className="mx-auto block w-full max-w-[320px]" role="img" aria-label="Six thinking hats radar">
        ...
        {axes.map((a) => {
          ...
          return <line key={a.hat.key} x1={c} y1={c} x2={x} y2={y} stroke="#2a2420" strokeOpacity={0.13} strokeWidth={1} />
        })}
        ...
        {/* axis labels */}
        {axes.map((a) => {
          const [lx, ly] = at(a.ux, a.uy, R + 16)
          const anchor = a.ux > 0.3 ? 'start' : a.ux < -0.3 ? 'end' : 'middle'
          return (
            <text key={`l${a.hat.key}`} x={lx} y={ly + 3.5} textAnchor={anchor} fontSize={11} fontWeight={700} fill="#2a2420">
