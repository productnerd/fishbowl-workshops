...
export interface ArchetypeResult extends Archetype {
 runnerUp: string
 runnerUpEssence: string // the runner-up's one-line essence, so we can explain it too
 fromSelf: boolean
...
export function deriveArchetype(
 ...
 return { ...ARCHETYPES[scores[0].key], runnerUp: ARCHETYPES[scores[1].key].name, runnerUpEssence: ARCHETYPES[scores[1].key].essence, fromSelf: Boolean(bigFive), drivers }
}
